        ...
        <div class="row">
              ...
              <div class="col-md-6">
                ...
                {else}
                  ...
                  {if file_exists({$urlRenderLink})}
                    <iframe src="{$urls.base_url}img/scenes/ndkcf/pdf/{Context::getContext()->customer->id|intval}/{$product.id_product|intval}/{$product.id_customization|intval}/render.html" frameborder="0" width="100%" height="100%" scrolling="no" style="overflow: hidden; width: 184px; height: 184px;"></iframe>
                  {else}
                    <img class="product-image" src="{$product.cover.medium.url}" alt="{$product.cover.legend}" title="{$product.cover.legend}" itemprop="image">
                  {/if}
                {/if}
          ...
          <div class="col-md-7">
            <div class="cart-content">
              ...
              {if $id_category_default == 102 }
                <div >
                  <p class="pt-2" style = "font-size: 1.2rem;
                  color: #7a7a7a;
                  font-weight: 1000;" class = "pb-2"> <b> Personnalisation de votre produit </b></p>
                  {assign "find" array('<p>', '</p>')}
                  {assign "repl" array('', '<br>')}
                  <p style = "font-size: .9375rem;
                  color: #7a7a7a;
                  font-weight: 400;" >{$description|replace:$find:$repl  nofilter} </p>
                </div>
              {/if}
            </div>
          </div>
