  ...
  <div class="product-prices">
      ...
      {if $product.has_discount}
        <div class="product-discount">
          ...
          {if {$infoProduto['cont_ndk']} > 0}
            <span class="regular-price" style="display: none;">{$product.regular_price}</span>
          {else}
    ...
    {block name='product_price'}
      <div class="{* product-price*} h5 {if $product.has_discount}has-discount{/if}" itemprop="offers" itemscope
        ...
        <meta itemprop="priceValidUntil" content="{$checaDescontosCatalogo['to']}" />
        ...
        {if {$infoProduto['cont_ndk']} > 0}
          <meta itemprop="price" content="{$precoAtualizadoSEO['preco_com_desconto_catalogo']}">
        {else}
          <meta itemprop="price" content="{$product.price_amount}">
        {/if}

        <div class="current-price mobile_padding">
          ...
          {* {if {$infoProduto['cont_reducao']} > 0} *}
            {if {$product.id} != '1120'}
              ...
              {else}
                {* <span itemprop="price" content="{$product.price_amount}">{$product.price}</span> *}
                <span content="{$product.price_amount}" class="current_price_aluclass">{$product.price}</span>
              {/if}
              {* <input type="text" value="{$infoProduto['cont_reducao']}"> *}
            {else}
              <span content="{$product.price_amount}" class="current_price_aluclass">{$product.price}</span>
            {/if}
            ...
            {if {$product.id} != '1120'}
              {if {$checaDescontosCatalogo['reduction']} >= 1}
                ...
                <span class="discount discount-percentage current_discount_aluclass">-{$checaDescontosCatalogo['reduction']}
                  AUJOURD'HUI</span>


      {* <span class="discount discount-percentage current_discount_aluclass">{l s='Save %percentage%' d='Shop.Theme.Catalog' sprintf=['%percentage%' => $checaDescontosCatalogo['reduction']]}</span> *}
      {/if}
      {/if}
      {* {if $product.has_discount}
      ...
      <span class="factoryDirectPrice">Prix direct usine (TTC)</span>
    </div>
