...
{$arrayidproduct = [595438]}


<div class="form-group ndkackFieldItem aluclass-disable-div {$influences[$field.id_ndk_customization_field]}"
  data-rposition="{$field.ref_position|escape:'intval'}" data-typefield="{$field.type|escape:'intval'}"
  data-position="{$field.position|escape:'intval'}" data-iteration="{$field_iteration}"
  data-id="{$field.target|escape:'htmlall':'UTF-8'}" data-view="{$field.target_child|escape:'htmlall':'UTF-8'}"
  data-field="{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}">
  ...
  <div class="fieldPane clearfix" style="display: none;">
      ...
      {if isset($features) && $features}
        {foreach from=$features item=feature}
          {if $feature.id_feature == $field.feature && isset($feature.value)}
            <span class="radio">
              <input data-message="{l s='Informe' mod='ndk_advanced_custom_fields'} {$field.name|escape:'htmlall':'UTF-8'}"
                id="radio_{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}_0" type="radio"
                name="ndkcsfield[{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}]"
                class="{if $field.required == 1} required_field{/if} ndk-radio not_uniform{if $field.is_visual == 1}visual-effect {/if}"
                data-group="{$field.id_ndk_customization_field|escape:'intval'}" data-src=""
                data-zindex="{$field.zindex|escape:'htmlall':'UTF-8'}" value="{$feature.value|escape:'htmlall':'UTF-8'}"
                data-price="0" checked="checked" data-id="{$field.target|escape:'htmlall':'UTF-8'}"
              ...
              <label
                for="radio_{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}_0">{$feature.value|escape:'htmlall':'UTF-8'}({l s='by default'  mod='ndk_advanced_custom_fields'})</label>
            </span>
          {/if}
    ...
    {foreach from=$field.values item=value}
      ...
      {if $value.set_quantity == 0 || $value.quantity > 0}
        <span class="radio">
          <input data-message="{l s='Informe' mod='ndk_advanced_custom_fields'} {$field.name|escape:'htmlall':'UTF-8'}"
            id="radio_{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}_{$value.id|escape:'htmlall':'UTF-8'}"
            type="radio" name="ndkcsfield[{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}]"
            class=" {if $field.required == 1} required_field{/if} ndk-radio not_uniform{if $field.is_visual == 1}visual-effect {/if} {if $field.required == 1} required_field{/if}"
            data-group="{$field.id_ndk_customization_field|escape:'intval'}"
            data-src="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|escape:'intval'}-thickbox_default.jpg"
            ...
            data-quantity-available="{if $value.set_quantity >0}{$value.quantity}{else}999999999{/if}"
            data-hide-field="{if $value.influences_restrictions|strpos:"all" !== false}1{else}0{/if}"
            data-id-value="{$value.id|escape:'intval'}" value="{$value.value|escape:'htmlall':'UTF-8'}"
            data-price="{if $valuePrice > 0}{$valuePrice|escape:'htmlall':'UTF-8'}{else}{$fieldPrice|escape:'htmlall':'UTF-8'}{/if}"
            data-id="{$field.target|escape:'htmlall':'UTF-8'}" data-view="{$field.target_child|escape:'htmlall':'UTF-8'}" />
          <label
            for="radio_{$field.id_ndk_customization_field|escape:'htmlall':'UTF-8'}_{$value.id|escape:'htmlall':'UTF-8'}">{$value.value|escape:'htmlall':'UTF-8'}
            <span id="valueradio_{$value.id|escape:'htmlall':'UTF-8'}">
              {if $valuePrice > 0} : {l s="+" mod='ndk_advanced_custom_fields'}
                ...
                {else}
                  {if $reduction_value < 100}
                    ...
                    {if in_array($product_id, $arrayidproduct)}
                      <s>{convertPrice price=($valuePrice-($valuePrice*($reduction_value/100)))*2}</s>
                    {else}
  ...
  <input type="hidden" id="ndkcsfieldPdf_{$field.id_ndk_customization_field|escape:'intval'}"
    name="ndkcsfieldPdf[{$field.id_ndk_customization_field|escape:'intval'}]" />
