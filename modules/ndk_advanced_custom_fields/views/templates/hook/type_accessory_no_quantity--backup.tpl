			...
			{foreach from=$field.values item=value}
				...
				<li class="col-xs-6 clearfix product_{$field.id_ndk_customization_field|escape:'intval'}_{$value.id_product_value} accessory-ndk accessory-ndk-no-quantity {if $field.is_visual == 1}visual-effect {/if} filterTag {if $value.tags && $value.tags !=''} tagged {foreach from=$tags item=tag}{$tag|replace:' ':'-'} {/foreach}{/if}" data-tags="{foreach from=$tags item=tag}{$tag}|{/foreach}"  data-value="{$value.value|escape:'htmlall':'UTF-8'}" title="{$value.value|escape:'htmlall':'UTF-8'}"  data-src="{if $value.is_image}{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|escape:'intval'}-{Configuration::get('NDK_IMAGE_LARGE_SIZE')}.jpg{else}0{/if}" data-group="{$field.id_ndk_customization_field|escape:'intval'}"  data-zindex="{$field.zindex|escape:'htmlall':'UTF-8'}"
				  ...
				  <div class="accessory_img_block clear clearfix " id="img_div_{$value.id|escape:'htmlall':'UTF-8'}">
				  	<img loading="lazy" class="img-responsive img_div_acess" id="img_{$value.id|escape:'htmlall':'UTF-8'}" src="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|escape:'intval'}-small_default.jpg"/>
				 	...
				 	<div class="price custumerprice_{$value.id|escape:'intval'}" id="price_{$value.id|escape:'intval'}">
				 		{if $valuePrice > 0}
				 			{if $field.price_type == 'percent'}
				 				+{$valuePrice}%
				 			{else}
								{if $field.id_ndk_customization_field|in_array:$arrayidfield}
									(<spam style="color: red;"><s> {convertPrice price=$valuePrice+150}</s></spam>)
								{/if}
								{convertPrice price=$valuePrice}
				 			{/if}
				 		{else}
				 			{if $fieldPrice > 0} :
				 				...
				 				{else}
									{if $field.id_ndk_customization_field|in_array:$arrayidfield}
										(<spam style="color: red;"><s> {convertPrice price=$fieldPrice+150}</s></spam>)
									{/if}
									{convertPrice price=$fieldPrice}
				 				{/if}
				 			{/if}
