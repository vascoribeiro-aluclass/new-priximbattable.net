		...
		<div class="fieldPane clearfix" style="display: none;">
			...
			<div class="clearfix" id="main-{$field.id_ndk_customization_field|escape:'intval'}">
				...
				{foreach from=$field.values item=value}
					...
					<li class="color-ndk colorize-ndk {if $field.is_visual == 1}visual-effect {/if}" data-value="{$value.id|escape:'intval'}" title="{$value.value|escape:'htmlall':'UTF-8'}"  data-src="" data-group="{$field.id_ndk_customization_field|escape:'intval'}"  data-zindex="{$field.zindex|escape:'htmlall':'UTF-8'}"
					...
					data-color="{if $value.is_texture}{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/{$value.id|escape:'intval'}-texture.jpg{/if} {$value.color|escape:'htmlall':'UTF-8'}"
