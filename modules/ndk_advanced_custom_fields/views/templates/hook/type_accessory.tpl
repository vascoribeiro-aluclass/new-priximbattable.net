    ...
    <ul class="ndk_accessory_list">
      {foreach from=$field.values item=value}
        ...
        {if $value.set_quantity == 0 || $value.quantity > 0}
          ...
          <li
            class="clearfix accessory-ndk {if $field.is_visual == 1}visual-effect {/if} filterTag {if $value.tags && $value.tags !=''} tagged {foreach from=$tags item=tag}{$tag|replace:' ':'-'} {/foreach}{/if}"
            data-tags="{foreach from=$tags item=tag}{$tag}|{/foreach}" data-value="{$value.id|escape:'intval'}"
            title="{$value.value|escape:'htmlall':'UTF-8'}"
             data-title="{$value.value|escape:'htmlall':'UTF-8'}"
              ...
              <div id="accessory-popup-{$value.id|escape:'intval'}" class="accessory-popup-ndk">
                {if $value.is_image}
                  <div class="col-md-6 ndk-img-block">
                    <img loading="lazy" class="img-responsive"
                      src="{if isset($is_https) && $is_https}{$base_dir_ssl}{else}{$base_dir}{/if}img/scenes/ndkcf/thumbs/{$value.id|escape:'intval'}-{Configuration::get('NDK_IMAGE_LARGE_SIZE')}.jpg" />
                  </div>
                {/if}
                <div class="col-sm-10 ndk-infos-block">
                  ...
                  <div class="price custumerprice_{$value.id|escape:'intval'}">
                    ...
                    {$arrayidfield = [5467,5473,5465]}
                    {$valuePrice2 = $valuePrice}
                    ...
                    {if $valuePrice > 0} :
                      ...
                      {else}
                        {if $reduction_value < 100}
                          ...
                          {else}
                            <s>{convertPrice price=$valuePrice2}</s>
                            <span style="color: var(--primary);">
                              {convertPrice price=$valuePrice2-($valuePrice2*($reduction_value/100))}</span>
                          {/if}
                        {else}
                          {convertPrice price=$valuePrice}
                        {/if}
            ...
            <div class="col-md-8 accessory-infos">
              ...
              <p class="ndk-accessory-quantity-block">
                ...
                <input type="number"
                  name="ndkcsfield[{$field.id_ndk_customization_field|escape:'intval'}][quantity][{$value.id|escape:'intval'}]"
                  ...
                  min="{$value.quantity_min|escape:'intval'}" type="text" class="ndk-accessory-quantity price_overrided"
                  ...
                  data-id-value="{$value.id|escape:'intval'}" data-value="{$value.id|escape:'intval'}"
