      ...
      <span class="tooltipDescMark">
		...
		<div class="fieldPane clearfix" style="display: none;">
			...
			{foreach from=$field.values item=value}
				...
				<li class="clearfix accessory-ndk filterTag tagged {foreach from=$tags item=tag}{$tag|replace:' ':'-'} {/foreach}" data-tags="{foreach from=$tags item=tag}{$tag}|{/foreach}"  data-group="{$field.id_ndk_customization_field|escape:'intval'}"  data-zindex="{$field.zindex|escape:'htmlall':'UTF-8'}"
				 ...
				 <div class="col-md-8 accessory-infos">
				 	...
				 	<span>{l s='Total quantity : ' mod='ndk_advanced_custom_fields'}</span>
				 		<input class="ndkcf_totalprod_quantity" id="ndkcf_totalprod_quantity_{$value.id|escape:'intval'}" type="text" readonly="readonly" value="0" name="totalprodquantity-{$value.id|escape:'intval'}-{$value.id_product_value|escape:'intval'}" size="8"/>
				 			...
				 			<div class="combColumn_list">
				 				...
				 				<span class="comblist_name" title="{$col.attribute_name}">
				 							{if $col.is_color_group == 1}
				 								...
				 								style="background: {if $img_color_exists}url('{$img_col_dir}{$col.id_attribute|intval}.jpg'){else}{$col.attribute_color|escape:'htmlall':'UTF-8'}{/if};">{$col.attribute_name}</span>
				 								<span class="ndk_attribute_name" style="color: {$col.attribute_color|escape:'htmlall':'UTF-8'};">{$col.attribute_name}</span>
				 							{else}
				 								{$col.attribute_name}
				 							{/if}
				 				</span>
				 				...
				 				{foreach $ndkcf_combinations.rows as $row}
				 					...
				 					<div class="combRow clear clearfix">
				 						<span class="ndkcf_row_title">
				 							<img class="img-responsive small_img" src="{$link->getImageLink($product.link_rewrite|escape:'html':'UTF-8', $id_image, Configuration::get('NDK_IMAGE_SIZE'))}"/>
				 							...
				 							{if NdkCf::class_exists_ndk('NdkAci')}
				 									...
				 									{if $combDesc}
				 										...
				 										<a class="fancybox accessory-more" href="#accessory-popup-{$value.id|escape:'intval'}-{$id_combination|escape:'intval'}"></a>

					 									<div style="display:none">
					 										 <div id="accessory-popup-{$value.id|escape:'intval'}-{$id_combination|escape:'intval'}" class="accessory-popup-ndk">
					 											 	<div class="col-md-6 ndk-img-block">
					 											 		<img class="img-responsive" src="{$link->getImageLink($product.link_rewrite|escape:'html':'UTF-8', $id_image, $ndk_image_size)|escape:'html':'UTF-8'}"/>
					 											 	</div>
					 										 	<div class="col-sm-6 ndk-infos-block">
					 										 		<p class="ndk-subtitle">{$row.attribute_name|escape:'htmlall':'UTF-8'}</p>
					 										 		<div class="ndk-accessory-desc">{$combDesc.short_description|escape:'UTF-8'}</div>
					 										 		<div class="ndk-accessory-desc">{$combDesc.description|escape:'UTF-8'}</div>

					 										 		<div class="price">
					 										 			<span class="final_price final_price_{$field.id_ndk_customization_field|escape:'intval'}-{$value.id|escape:'intval'}-{$id_combination|escape:'intval'}">{convertPrice price=NdkCf::getAttributePrice($value.id_product_value, $id_combination)}</span>
					 										 		</div>
					 										 		{include file='./quantity_discount.tpl'}
					 										 	</div>
					 										 </div>
