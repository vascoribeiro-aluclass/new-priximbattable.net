...
class OrderControllerCore extends FrontController
{
    ...
    public function displayAjaxCartListShow()
    {
      ...
      $objCart = new Cart( Context::getContext()->cart->id, (int)Context::getContext()->language->id);
      $total_price = $objCart->getOrderTotal(true, Cart::BOTH);

      foreach ($products as $product){
        $tax_manager = TaxManagerFactory::getManager($address, Product::getIdTaxRulesGroupByIdProduct((int)$product['id_product'], Context::getContext()));
        $product_tax_calculator = $tax_manager->getTaxCalculator();

        $objProduct = new Product((int)$product['id_product'],false,(int)Context::getContext()->language->id);
        $templateParams['produts'][(int)$objProduct->id]['id'] = (int)$objProduct->id;
        $templateParams['produts'][(int)$objProduct->id]['name'] = $objProduct->name;
        $imgCover = $objProduct->getCover((int)$objProduct->id);
        $templateParams['produts'][(int)$objProduct->id]['image'] = $img_url;
        $link       =  new Link();
        ...
        $templateParams['produts'][(int)$objProduct->id]['image'] = $img_url;
        $templateParams['produts'][(int)$objProduct->id]['qty'] = $product['cart_quantity'];
        $templateParams['produts'][(int)$objProduct->id]['price'] = ($product_tax_calculator->addTaxes($product['price']) *$product['cart_quantity']);
      }
      $templateParams['price_total'] = $total_price;

      ob_end_clean();
      header('Content-Type: application/json');
      $this->ajaxRender(Tools::jsonEncode(array(
        'cart_list' => $this->render(
            'checkout/_partials/cart_list',
            $templateParams
        ),
      )));
