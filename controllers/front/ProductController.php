...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function initContent()
    {
        ...
        $this->registerStylesheet('aluclassic', '/assets/css/product.css', ['media' => 'all', 'priority' => 1]);
        ...
        if (!$this->errors) {
            ...
            $array_option_str_sur = $this->getdescriptionshort($this->product->id);
            $resultados = $this->getDadosDaBaseDeDados($this->product->id);
            ...
            $this->context->smarty->assign([
                ...
                'option_str_sur' => $array_option_str_sur,
                'videos' => $resultados,
            ]);
    ...
    public function getdescriptionshort($product_id)
    {
        ...
        if ($results = Db::getInstance()->executeS($querystandard)) {
            ...
            foreach ($results as $row) {
                $id_product_standard = $product_id;
                $id_product_surmesure = $row['id_product_surmesure'];
            }
        }
        $querysurmesure = 'SELECT * FROM `' . _DB_PREFIX_ . 'ndk_links_models` where `id_product_surmesure` = ' .  $product_id;
        if ($results = Db::getInstance()->executeS($querysurmesure)) {
            $active_product_surmesure = 1;
            foreach ($results as $row) {
                $id_product_surmesure  = $product_id;
                $id_product_standard = $row['id_product_standard'];
            }
        }
        ...
        $array_option_str_sur['link_product_standard'] = $product_standard->getLink();
        $array_option_str_sur['link_product_surmesure'] =  $product_surmesure->getLink();
        $array_option_str_sur['active_product_standard'] = $active_product_standard;
        $array_option_str_sur['active_product_surmesure'] =  $active_product_surmesure;
        $array_option_str_sur['query_product_standard'] =  $querystandard;
        $array_option_str_sur['query_product_surmesure'] =  $querysurmesure;
    ...
    private function getDadosDaBaseDeDados($product_id)
    {
        // Consulta SQL
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'video_gallery_product WHERE id_product = '.$product_id.'';
