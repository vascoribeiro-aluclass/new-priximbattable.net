...
class FrontControllerCore extends Controller
{
    ...
    public function GetScoreSites()
    {
        $array_nota = array();
        $notaprix = Db::getInstance()->executeS("SELECT ROUND(AVG(`grade`),1) as media_nota FROM `ps_product_comment`where `validate` = 1 AND `deleted` = 0 ");
        foreach ($notaprix as $row) {
            $array_nota['PRIXIMBATTABLE'] = $row['media_nota'];
        }

        $notagoogle = Db::getInstance()->executeS(" SELECT `nome`,`nota` FROM `ps_notas_externas`  ");
        foreach ($notagoogle as $row) {
            $array_nota[$row['nome']] = $row['nota'];
        }
    ...
    protected function assignGeneralPurposeVariables()
    {
        $templateVars = array(
            ...
            'scoreReview' => $this->GetScoreSites(),
    ...
    public function setMedia()
    {
        ...
        $this->registerStylesheet('theme-fontawesome', '/assets/css/font-awesome.css', ['media' => 'all', 'priority' => 1001]);
        ...
        if ($this->php_self == 'product') {
          $this->registerJavascript('product-js', '/assets/js/product.js', ['position' => 'bottom', 'priority' => 1001]);
        }
    ...
    protected function getAlternativeLangsUrl()
    {
        ...
        if(Tools::getValue('id_product')){
          ...
          $alternativelinksarray = Db::getInstance()->executeS('SELECT  `link`,`lang`
          FROM `' . _DB_PREFIX_ . 'link_orther_sites`
          where  `id` = '.Tools::getValue('id_product'));

          foreach( $alternativelinksarray as  $alternativelinkvalue ){
            $alternativeLangs[$alternativelinkvalue['lang']] = $alternativelinkvalue['link'];
          }
